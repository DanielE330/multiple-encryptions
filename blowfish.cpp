...
namespace {
    ...
    const quint32 P_INIT[18] = {0x243F6A88, 0x85A308D3, 0x13198A2E, 0x03707344, 0xA4093822, 0x299F31D0, 0x082EFA98, 0xEC4E6C89, 0x452821E6, 0x38D01377, 0xBE5466CF, 0x34E90C6C, 0xC0AC29B7, 0xC97C50DD, 0x3F84D5B5, 0xB5470917, 0x9216D5D9, 0x8979FB1B};

    const quint32 S_INIT[4][256] = {
        // S-блок 0
...
void Blowfish::initialize(const QString &strKey) {
    if (initialized) return ;

    for(int i = 0; i < 18; ++i) P[i] = P_INIT[i];

    for(int i = 0; i < 4; ++i)
        for (int j = 0; j < 256; ++j)
            S[i][j] = S_INIT[i][j];

    QByteArray keyBytes = strKey.toUtf8();
    for (int i = 0; i < 18; ++i) {
        quint32 keyPart = 0;
        for (int j = 0; j < 4; ++j)
            keyPart = (keyPart << 8) | static_cast<quint8>(keyBytes[(i * 4 + j) % keyBytes.size()]);
        P[i] ^= keyPart;
    }

    quint32 left = 0, right = 0;
...
quint32 Blowfish::feistel(quint32 &x) {
    quint8 a = (x >> 24) & 0xFF;
    quint8 b = (x >> 16) & 0xFF;
    quint8 c = (x >> 8) & 0xFF;
    quint8 d = x & 0xFF;
...
void Blowfish::encrypt_block(quint32 &left, quint32 &right) {
    for (int i = 0; i < 16; ++i) {
        left ^= P[i];
        right ^= feistel(left);
        std::swap(left, right);
...
void Blowfish::decrypt_block(quint32 &left, quint32 &right) {
    for (int i = 17; i > 1; --i) {
        left ^= P[i];
        right ^= feistel(left);
        std::swap(left, right);
...
QString Blowfish::encryption(const QString &str, const QString &strKey) {
    initialize(strKey);
    QByteArray data = str.toUtf8();

    int padSize = 8 - (data.size() % 8);
    data.append(padSize, static_cast<char>(padSize));

    for (int i = 0; i < data.size(); i += 8) {
        quint32 left = qFromBigEndian<quint32>(data.constData() + i);
        quint32 right = qFromBigEndian<quint32>(data.constData() + i + 4);
        encrypt_block(left, right);
        qToBigEndian(left, data.data() + i);
        qToBigEndian(right, data.data() + i + 4);
    }

    return QString::fromLatin1(data.toBase64());
}

QString Blowfish::decryption(const QString &str, const QString &strKey) {
    initialize(strKey);
    ...
    int padSize = static_cast<quint8>(data[data.size() - 1]);
    if (padSize <= 8) data.chop(padSize);
